/* ===== EPIOS — Générateur PDF de marque (jsPDF) =====
   Style aligné au site : palette rouge EPIOS, bandeau d'accent, mise en page soignée.
   Expose window.EP.buildDevisPDF(d) et window.EP.buildRdvPDF(d). */
(function () {
  const C = {
  ...
  function header(doc, badge, d) {
    ...
    // léger dégradé simulé : bande rouge EPIOS en bas du bandeau
    ...
    // pastille logo (fond blanc + E noir avec accent rouge)
    ...
    doc.text("Solutions informatiques \u00b7 Nice et alentours", M + 17, 22.5);
  ...
  function footer(doc) {
    ...
    // Identification légale de l'entreprise (obligatoire sur devis et factures)
    ...
    doc.text('Interventions \u00e0 distance & \u00e0 domicile \u2014 Nice et alentours (30 km). Prestations soumises aux conditions', M, 291.5);
  ...
  function buildDevisPDF(d) {
    ...
    doc.text('Prix net, TVA non applicable (art. 293 B du CGI). Composants \u00e9ventuels factur\u00e9s en sus, sur devis s\u00e9par\u00e9.', M, y + 4.2);
